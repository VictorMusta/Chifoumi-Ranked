...
export class TeamsController {
  ...
  async getMyTeam(@Req() req: any) {
    ...
    if (!ownerId) {
      console.error('[TeamsController] Request user object is missing ID:', req.user);
    ...
    if (!team) {
      ...
      const starterAthletes = await this.createStarterSquadUseCase.execute(teamId);
      
      team = new Team(teamId, ownerId, 1000, starterAthletes);
      await this.teamRepo.save(team);
      console.log('[TeamsController] Team created with', starterAthletes.length, 'athletes');
    }
